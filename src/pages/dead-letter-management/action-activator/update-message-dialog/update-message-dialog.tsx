...
import {
  useO5DanteV1DeadMessageCommandServiceUpdateDeadMessage,
...
export function UpdateMessageDialog({ messageId }: UpdateMessageDialogProps) {
  ...
  const { isPending, error } = useO5DanteV1DeadMessageCommandServiceUpdateDeadMessage();
  useErrorHandler(error, 'Error updating message');

  const { data, error: messageError } = useO5DanteV1DeadMessageQueryServiceGetDeadMessage(isOpen ? { messageId } : undefined);
      ...
      message: {
        payload: {
          json: formatJSONString(
            data?.message?.data?.currentVersion?.message?.body?.value ||
              `{
