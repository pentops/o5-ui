...
export function buildO5AwsDeployerV1DeploymentQueryServiceListDeploymentEventsKey(
  request?: O5AwsDeployerV1DeploymentQueryServiceListDeploymentEventsRequest,
) {
  const { page, ...rest } = request || {};
  if (rest) return ['O5AwsDeployerV1DeploymentEvent', 'list', rest] as const;
  return ['O5AwsDeployerV1DeploymentEvent'] as const;
}
...
export function useO5AwsDeployerV1DeploymentQueryServiceListDeploymentEvents(
  ...
  return useInfiniteQuery({
    queryKey: buildO5AwsDeployerV1DeploymentQueryServiceListDeploymentEventsKey(request),
    queryFn: async ({ pageParam }) =>
      o5AwsDeployerV1DeploymentQueryServiceListDeploymentEvents(
        '',
        request?.deploymentId ? { ...request, page: pageParam ? { token: pageParam } : undefined } : undefined,
      ),
